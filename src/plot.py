...
def plot_from_list(list_PATH, metric, list_title, vlimit):
    ...
    for fig_idx, PATH in enumerate(list_PATH):
        ...
        if not subplot_title:
            if order == "ad":
                subplot_title = f"metric={metric}, q_type={q_type}, peer_type={d_type}, order=agree first"
            elif order == "da":
                subplot_title = f"metric={metric}, q_type={q_type}, peer_type={d_type}, order=disagree first"
...
def plot_from_list_data(list_data, metric, list_title, vlimit):
    ...
    for fig_idx, results in enumerate(list_data):
        cbar = True if fig_idx == num_plots - 1 else False
