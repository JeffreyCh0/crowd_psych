...
def mmlu_eval_one(mmlu_input, disagree_type, num_workers = mp.cpu_count()):
    with mp.Pool(num_workers) as pool:
        results = list(tqdm(pool.imap(process_one, [(input_ele, disagree_type) for input_ele in mmlu_input]), total=len(mmlu_input), desc="Processing MMLU"))
    return results
