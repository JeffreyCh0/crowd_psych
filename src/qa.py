...
def process_grp_discrete(args):
    ...
    input_ele, use_reason, agree_size, disagree_size, disagree_type, order = args
...
def process_grp_education(args):
    ...
    for row in input_feat_list:
        for eval_feat in row:
            ...
            if eval_type == 'grp_count':
                ...
                input_list.extend([(input_ele, agree_size, disagree_size, disagree_type, order) for input_ele in qa_input])
                ...
                disagree_type = eval_feat['disagree_type']
                order = eval_feat['order']
                input_list.extend([(input_ele, use_reason, agree_size, disagree_size, disagree_type, order) for input_ele in qa_input])

    print(f"Processing {eval_type} samples...")
    with mp.Pool(num_workers) as pool:
        flatten_results = list(tqdm(pool.imap(func, input_list), total=len(input_list), desc="Processing QA"))
...
def qa_eval_education(qa_input, disagree_type, num_workers = mp.cpu_count()):
    ...
    with mp.Pool(num_workers) as pool:
        results = list(tqdm(pool.imap(process_grp_education, input_list), total=len(input_list), desc="Processing QA"))
